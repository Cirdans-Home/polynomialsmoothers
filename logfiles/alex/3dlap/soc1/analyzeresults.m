...
[~,logfiles] = system('ls -v1 4/log_cheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_cheby4 = maketable(logfiles);

TSOLVE1(1,:) = tab_soc1_cheby4.t_solve(:);
TITER1(1,:) = tab_soc1_cheby4.t_it(:);
NGPUS = tab_soc1_cheby4.np(:);
%% V-SVBM-4OPTCHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 4/log_optcheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby4 = maketable(logfiles);

TSOLVE1(2,:) = tab_soc1_optcheby4.t_solve(:);
TITER1(2,:) = tab_soc1_optcheby4.t_it(:);
%% V-SVBM-4OPTCHEBY1-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 4/log_optcheby1_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby1 = maketable(logfiles);

TSOLVE1(3,:) = tab_soc1_optcheby1.t_solve(:);
TITER1(3,:) = tab_soc1_optcheby1.t_it(:);
%% V-SVBM-4L1-JAC-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 4/log_l1jacobi_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_l1jac = maketable(logfiles);

TSOLVE1(4,:) = tab_soc1_l1jac.t_solve(:);
TITER1(4,:) = tab_soc1_l1jac.t_it(:);
%% Plot analysis
figure(1)
subplot(3,2,1)
...
%% V-SVBM-4CHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 6/log_cheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_cheby4 = maketable(logfiles);

TSOLVE2(1,:) = tab_soc1_cheby4.t_solve(:);
TITER2(1,:) = tab_soc1_cheby4.t_it(:);
NGPUS = tab_soc1_cheby4.np(:);
%% V-SVBM-4OPTCHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 6/log_optcheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby4 = maketable(logfiles);

TSOLVE2(2,:) = tab_soc1_optcheby4.t_solve(:);
TITER2(2,:) = tab_soc1_optcheby4.t_it(:);
%% V-SVBM-4OPTCHEBY1-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 6/log_optcheby1_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby1 = maketable(logfiles);

TSOLVE2(3,:) = tab_soc1_optcheby1.t_solve(:);
TITER2(3,:) = tab_soc1_optcheby1.t_it(:);
%% V-SVBM-4L1-JAC-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 6/log_l1jacobi_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_l1jac = maketable(logfiles);

TSOLVE2(4,:) = tab_soc1_l1jac.t_solve(:);
TITER2(4,:) = tab_soc1_l1jac.t_it(:);
%% Plot analysis
figure(1)
subplot(3,2,2)
...
%% V-SVBM-4CHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 8/log_cheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_cheby4 = maketable(logfiles);

TSOLVE3(1,:) = tab_soc1_cheby4.t_solve(:);
TITER3(1,:) = tab_soc1_cheby4.t_it(:);
NGPUS = tab_soc1_cheby4.np(:);
%% V-SVBM-4OPTCHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 8/log_optcheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby4 = maketable(logfiles);

TSOLVE3(2,:) = tab_soc1_optcheby4.t_solve(:);
TITER3(2,:) = tab_soc1_optcheby4.t_it(:);
%% V-SVBM-4OPTCHEBY1-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 8/log_optcheby1_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby1 = maketable(logfiles);

TSOLVE3(3,:) = tab_soc1_optcheby1.t_solve(:);
TITER3(3,:) = tab_soc1_optcheby1.t_it(:);
%% V-SVBM-4L1-JAC-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 8/log_l1jacobi_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_l1jac = maketable(logfiles);

TSOLVE3(4,:) = tab_soc1_l1jac.t_solve(:);
TITER3(4,:) = tab_soc1_l1jac.t_it(:);
%% Plot analysis
figure(1)
subplot(3,2,3)
...
%% V-SVBM-4CHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 10/log_cheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_cheby4 = maketable(logfiles);

TSOLVE4(1,:) = tab_soc1_cheby4.t_solve(:);
TITER4(1,:) = tab_soc1_cheby4.t_it(:);
NGPUS = tab_soc1_cheby4.np(:);
%% V-SVBM-4OPTCHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 10/log_optcheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby4 = maketable(logfiles);

TSOLVE4(2,:) = tab_soc1_optcheby4.t_solve(:);
TITER4(2,:) = tab_soc1_optcheby4.t_it(:);
%% V-SVBM-4OPTCHEBY1-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 10/log_optcheby1_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby1 = maketable(logfiles);

TSOLVE4(3,:) = tab_soc1_optcheby1.t_solve(:);
TITER4(3,:) = tab_soc1_optcheby1.t_it(:);
%% V-SVBM-4L1-JAC-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 10/log_l1jacobi_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_l1jac = maketable(logfiles);

TSOLVE4(4,:) = tab_soc1_l1jac.t_solve(:);
TITER4(4,:) = tab_soc1_l1jac.t_it(:);
%% Plot analysis
figure(1)
subplot(3,2,4)
...
%% V-SVBM-4CHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 12/log_cheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_cheby4 = maketable(logfiles);

TSOLVE5(1,:) = tab_soc1_cheby4.t_solve(:);
TITER5(1,:) = tab_soc1_cheby4.t_it(:);
NGPUS = tab_soc1_cheby4.np(:);
%% V-SVBM-4OPTCHEBY4-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 12/log_optcheby4_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby4 = maketable(logfiles);

TSOLVE5(2,:) = tab_soc1_optcheby4.t_solve(:);
TITER5(2,:) = tab_soc1_optcheby4.t_it(:);
%% V-SVBM-4OPTCHEBY1-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 12/log_optcheby1_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_optcheby1 = maketable(logfiles);

TSOLVE5(3,:) = tab_soc1_optcheby1.t_solve(:);
TITER5(3,:) = tab_soc1_optcheby1.t_it(:);
%% V-SVBM-4L1-JAC-30l1JAC CPU/GPU
[~,logfiles] = system('ls -v1 12/log_l1jacobi_soc1*');
logfiles = split(logfiles);
logfiles(end) = [];
tab_soc1_l1jac = maketable(logfiles);

TSOLVE5(4,:) = tab_soc1_l1jac.t_solve(:);
TITER5(4,:) = tab_soc1_l1jac.t_it(:);
%% Plot analysis
figure(1)
subplot(3,2,5)

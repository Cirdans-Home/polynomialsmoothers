...
do
echo "run GPU with ${smoother}"

./anisopsblascuda >> soc1/log_${smoother}_svbm_l1jac_${idim}_task_1_thr_24_GPU.txt 2>&1 <<EOF
...
for smoother in ${smoothers[@]}
do
echo "run CPU with ${smoother}"

./anisopsblas >> soc1/log_${smoother}_svbm_l1jac_${idim}_task_1_thr_24_CPU.txt 2>&1 <<EOF
%%%%%%%%%%%  General  arguments % Lines starting with % are ignored.
